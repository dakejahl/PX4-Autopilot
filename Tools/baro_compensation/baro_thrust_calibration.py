...
Identifies optimal SENS_BARO_PCOEF and SENS_BARO_PTAU parameters by comparing
...
constant (seconds). The vehicle_air_data module applies:
baro_alt += SENS_BARO_PCOEF * filtered_thrust to cancel this error,
so SENS_BARO_PCOEF = -K.
...
Outputs:
    ...
    - Console output with recommended SENS_BARO_PCOEF / SENS_BARO_PTAU values
"""
...
def plot_calibration(calib):
    ...
    lines = [
        ...
        "",
        f"  SENS_BARO_PCOEF  = {calib['recommended_pcoef_thr']:+.2f}  m",
        f"  SENS_BARO_PTAU = {calib['recommended_pcoef_ttau']:.2f}  s",
        "",
...
def plot_scatter(baro_err, thrust_data, calib, hover_start, hover_end):
    ...
    # Raw
    ax = axes[0]
    ax.scatter(thrust_mag, err_hov, s=3, alpha=0.4, color="tab:orange")
    z = np.polyfit(thrust_mag, err_hov, 1)
    x_fit = np.linspace(thrust_mag.min(), thrust_mag.max(), 50)
    ax.plot(x_fit, np.polyval(z, x_fit), "k--", linewidth=1.2)
    r_val = float(np.corrcoef(thrust_mag, err_hov)[0, 1])
    ...
    ax.scatter(thrust_mag, comp_err, s=3, alpha=0.4, color="tab:blue")
    z = np.polyfit(thrust_mag, comp_err, 1)
    ax.plot(x_fit, np.polyval(z, x_fit), "k--", linewidth=1.2)
    r_val = float(np.corrcoef(thrust_mag, comp_err)[0, 1])
    ax.set_xlabel("Thrust magnitude [0-1]")
    ax.set_ylabel("Compensated Error [m]")
    ax.set_title(f"After Compensation "
                 f"(PCOEF={calib['recommended_pcoef_thr']:+.2f})\n"
...
def plot_validation(calib, existing_pcoef, existing_ptau, baro_err,
    ...
    thrust_raw = np.interp(err_t[hov], thrust_data["time_s"],
                           thrust_data["thrust_z"])
    thrust_mag = np.clip(-thrust_raw, 0, 1)
    thrust_filt = first_order_lpf(thrust_mag, err_t[hov], existing_ptau)
    ...
    r_raw = float(np.corrcoef(thrust_mag, raw_err_hov)[0, 1])
    r_comp = float(np.corrcoef(thrust_mag, comp_err_hov)[0, 1])
    ...
    lines = [
        ...
        f"    PTAU  = {calib['recommended_pcoef_ttau']:.2f}  "
...
def plot_scatter_validation(baro_err, thrust_data, hover_start, hover_end,
    ...
    thrust_filt = first_order_lpf(thrust_mag, t[hov], existing_ptau)
    comp_err = baro_err["error"][hov]
    raw_err = comp_err - existing_pcoef * thrust_filt

    # Left: Reconstructed raw error
    ax = axes[0]
    ax.scatter(thrust_mag, raw_err, s=3, alpha=0.4, color="tab:orange")
    z = np.polyfit(thrust_mag, raw_err, 1)
    x_fit = np.linspace(thrust_mag.min(), thrust_mag.max(), 50)
    ax.plot(x_fit, np.polyval(z, x_fit), "k--", linewidth=1.2)
    r_val = float(np.corrcoef(thrust_mag, raw_err)[0, 1])
    ax.set_xlabel("Thrust magnitude [0-1]")
    ax.set_ylabel("Baro Error [m]")
...
def main():
    parser = argparse.ArgumentParser(
        description="Calibrate barometer thrust compensation parameters")
    ...
    for p in ["EKF2_HGT_REF", "EKF2_BARO_CTRL", "EKF2_BARO_NOISE",
              "SENS_BARO_PCOEF", "SENS_BARO_PTAU"]:
        val = get_param(ulog, p)
        if val is not None:
            print(f"  {p:24s} = {val}")

    # Detect whether compensation was active during this flight
    ...
    if is_calibrated:
        print(f"\n  >> Compensation was ACTIVE during this flight")
        print(f"  >> Running in validation mode")
    ...
    if is_calibrated:
        # --- VALIDATION MODE ---
        print(f"\nObserved baro error (compensation active):")
        print(f"  Mean:  {np.mean(err_hov):+.3f} m")
        print(f"  Std:   {np.std(err_hov):.3f} m")

        # Reconstruct what the raw error would have been
        raw_baro_err = reconstruct_raw_error(
            baro_err, thrust_data, existing_pcoef, existing_ptau)
        raw_hov = raw_baro_err["error"][hov]
        print(f"\nReconstructed raw error (before compensation):")
        print(f"  Mean:  {np.mean(raw_hov):+.3f} m")
        print(f"  Std:   {np.std(raw_hov):.3f} m")

        # Thrust correlation for both
        thrust_interp_mag = np.clip(
            -np.interp(baro_err["time_s"][hov],
                       thrust_data["time_s"], thrust_data["thrust_z"]),
            0, 1)
        r_raw = float(np.corrcoef(thrust_interp_mag, raw_hov)[0, 1])
        r_comp = float(np.corrcoef(thrust_interp_mag, err_hov)[0, 1])
        print(f"\nThrust correlation:")
        print(f"  Reconstructed raw: r = {r_raw:+.3f}"
              f"  ({'strong' if abs(r_raw) > 0.6 else 'moderate' if abs(r_raw) > 0.3 else 'weak'})")
        print(f"  Compensated:       r = {r_comp:+.3f}"
              f"  ({'strong' if abs(r_comp) > 0.6 else 'moderate' if abs(r_comp) > 0.3 else 'weak'})")

        # Re-identify parameters from reconstructed raw
        print("\nRe-identifying parameters from reconstructed raw data ...")
        calib = calibrate(raw_baro_err, thrust_data, hover_start, hover_end)

        if calib is None:
            print("Calibration failed on reconstructed data.",
                  file=sys.stderr)
            sys.exit(1)

        print(f"\n  Re-identified K         = {calib['best_K']:.3f} m/unit thrust")
        print(f"  Re-identified tau       = {calib['best_tau']:.3f} s")
        print(f"  Model R\u00b2               = {calib['best_r2']:.3f}")

        # Variance reduction
        raw_var = float(np.var(raw_hov))
        comp_var = float(np.var(err_hov))
        var_reduction = ((1.0 - comp_var / raw_var) * 100
                         if raw_var > 0 else 0)

        print(f"\n{'='*50}")
        print(f"  VALIDATION RESULT")
        print(f"{'='*50}")
        print(f"  Active:        PCOEF = {existing_pcoef:+.2f},"
              f"  PTAU = {existing_ptau:.2f}")
        print(f"  Re-identified: PCOEF = {calib['recommended_pcoef_thr']:+.2f},"
              f"  PTAU = {calib['recommended_pcoef_ttau']:.2f}")
        print(f"  Variance reduced by {var_reduction:.1f}%")
        print(f"  Residual thrust |r| = {abs(r_comp):.3f}")
        print(f"{'='*50}")

        if abs(r_comp) < 0.3:
            print(f"\n  Compensation is effective. No adjustment needed.")
        else:
            print(f"\n  Significant residual thrust correlation detected.")
            print(f"  Consider updating to re-identified values.")
        ...
        figures = [
            plot_overview(baro_err, thrust_data, armed_start, armed_end,
                          hover_start, hover_end),
            plot_validation(calib, existing_pcoef, existing_ptau, baro_err,
                            thrust_data, hover_start, hover_end),
            plot_scatter_validation(baro_err, thrust_data, hover_start,
                                   hover_end, existing_pcoef, existing_ptau),
        ]
        ...
        print(f"{'='*50}")
        print(f"  SENS_BARO_PCOEF  = {calib['recommended_pcoef_thr']:+.2f}")
        print(f"  SENS_BARO_PTAU = {calib['recommended_pcoef_ttau']:.2f}")
        print(f"{'='*50}")
        ...
        if improvement < 0.01:
            print(f"\nNote: The lag filter provides negligible improvement over "
                  "tau=0. You may set SENS_BARO_PTAU = 0.")
